class OptionsManager {
  ...
  getThemeSettings() {
    ...
    const base = window.PixelThemes.mergeTheme(presets[baseKey] || presets.pink);
    return {
  ...
  applyI18n() {
    ...
    window.PixelI18n.applyMap(document, language, {
      text: {
        ...
        '.settings-nav a[href="#donate"]': 'donate',
  ...
  updateSectionNumbers() {
    ['behavior', 'theme', 'donate', 'language', 'data', 'about'].forEach((id, index) => {
  ...
  updateThemePresetLabels(selectId) {
    ...
    if (!select) return;
    const lang = window.PixelI18n.normalize(this.settings.language || 'zh-CN');
